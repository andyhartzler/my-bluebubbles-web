// ============================================================
// EDGE FUNCTION: analyze-bills-batch-check — RETIRED 2026-08-23
// ============================================================
//
// This function polled the ANTHROPIC MESSAGE BATCHES API (GET /v1/messages/batches/{id} and
// /results) for batches that analyze-bills-batch-submit had queued, then wrote the
// per-bill analysis rows back.
//
...
//
// 2. ITS OTHER HALF IS GONE. analyze-bills-batch-submit is retired in the same change,
...
//    Its cron job (analyze-bills-batch-check, every minute) is INACTIVE and its last
//    recorded run predates the current job_run_details retention. With no submitted or
//    processing rows in public.legislation_ai_batches since 2026-01-19, every run it
//    did make returned "No analysis batches to check" without calling Anthropic at all,
//    so it was burning cron slots rather than tokens.
//
...
// removes the door.
//
// THE SUPPORTED PATH IS analyze-bills-batch.
//
...
serve((req) => {
  ...
  return new Response(
    JSON.stringify({
      error: "analyze-bills-batch-check is retired",
