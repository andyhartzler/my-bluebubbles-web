...
async function getUserInfoFromSubscriberId(subscriberId: string) {
  const { data, error } = await supabase
    .from('subscribers')
    .select('email, member_id, donor_id')
    .eq('id', subscriberId)
    .single();
...
async function getNameFromMemberId(memberId: string | null) {
...
// Rate limiting for auto shortcuts (max 10 per day per subscriber)
async function checkAutoShortcutRateLimit(subscriberId: string): Promise<number> {
  ...
  if (error) {
    ...
    return 0; // Allow if error checking
  }
  
  return data?.length || 0; // Returns count of emails sent today
}
...
serve(async (req) => {
  ...
  try {
    // ONLY requires subscriber_id - everything else is looked up
    ...
    const emailsSentToday = await checkAutoShortcutRateLimit(subscriber_id);
    if (emailsSentToday >= 10) {
      return new Response(JSON.stringify({
        error: "Daily limit reached (10 emails per day). Try again tomorrow.",
        rateLimited: true,
    ...
    const userInfo = await getUserInfoFromSubscriberId(subscriber_id);
    const name = await getNameFromMemberId(userInfo.member_id) || "A Concerned Missouri Resident";
    const email = userInfo.email;

    // Random parameters for uniqueness
    const seed = Math.floor(Math.random() * 1000000);
    const timestamp = Date.now();
    const targetWordCount = Math.floor(Math.random() * 76) + 100; // 100-175 words

    // Randomly select a tone
    const tones = ["concerned", "disappointed", "frustrated", "firm", "passionate"];
    ...
    const selectedTone = toneDescriptions[randomTone];
...
Generate the email body only. No subject line. No "Dear Attorney General" greeting. Start directly with the content.`;
    ...
    // Generate email body with duplicate checking
    let attempt = 0;
    let emailBody = "";
...
Generate ONLY the subject line text, no quotes or extra formatting.`;
    ...
    const trackingData = {
      ...
      user_zip_code: null, // Not collected in auto mode
      user_tone: randomTone,
      ...
      send_method: "auto_shortcut",
      is_shortcut_generated: true,
    ...
    // Return email data for the Shortcut to send
    return new Response(JSON.stringify({
      subject,
      body: emailBody,
      ...
      metadata: {
        ...
        tone_used: randomTone,
        user_name: name,
        is_member: !!userInfo.member_id,
        is_donor: !!userInfo.donor_id,
        emails_sent_today: emailsSentToday + 1,
        emails_remaining_today: 10 - (emailsSentToday + 1)
      }
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" }
    });
