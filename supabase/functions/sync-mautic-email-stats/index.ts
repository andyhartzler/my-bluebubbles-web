...
serve(async (req) => {
  ...
  try {
    ...
    const { emailId, syncAll = false, limit = 100 } = await req.json();
    ...
    if (emailId) {
      ...
      // Sync all emails
      console.log("Syncing all emails from Mautic...");
      let start = 0;
      let hasMore = true;
      ...
      throw new Error("Either emailId or syncAll must be provided");
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        message: `Successfully synced ${totalCampaigns} campaign(s) from Mautic. Note: Mautic API does not provide individual recipient stats - only aggregate sentCount/readCount are available.`,
      }),
