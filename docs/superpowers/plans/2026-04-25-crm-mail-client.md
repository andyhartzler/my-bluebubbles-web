...
- ⏳ **step 4** (next major chunk, ~1-2 sessions): bridge tmail's JMAP data sources to our edge functions. tmail's `EmailDataSource` interface alone has 35+ methods. Phase 1 critical subset: `getEmailContent` (→ mail-message-get), `sendEmail` (→ mail-send), `markAsRead` (no-op for now). Other 32 methods can throw `UnimplementedError`. Same approach for `ThreadDataSource`, `MailboxDataSource` (return synthetic INBOX/SENT mailbox tree), `SessionDataSource` (return synthetic Session for our auth). Once these adapters exist, bind them at the GetX binding level in place of the JMAP impls — then tmail's full `MailboxDashBoard` / `EmailView` / `ComposerView` widget trees render and function end-to-end with our edge fns as the backend.
